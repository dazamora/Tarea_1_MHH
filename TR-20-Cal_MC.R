...
PrepT<-read.table(paste(getwd(),"DATOS_AGUACERO",data[3],sep="/"),h=F)
...
Parameters<-matrix(0,itermax,4);

colnames(Parameters)<-c("CN","Lambda","Tc","Otra")
...
for(i in 1:itermax){
  ...
  if(length(rule1)!=length(Pc)){
    ...
    print("Perdidas iniciales")
    print(i)

  }
  
  out1<-Pec[1:end(Pec)[1]-1,2]-Pec[2:end(Pec)[1],2]
